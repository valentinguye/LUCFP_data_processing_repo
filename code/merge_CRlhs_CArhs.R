...
merge_lhs_rhs <- function(parcel_size, catchment_radius){
  ...
  if(!all.equal(sort(unique(bfe$lonlat)), sort(unique(LHS$lonlat)))){stop("baseline forest extent and main data frames do not have the same set of grid cells")}  
  # merge it with our parcel panel
  LHS <- left_join(LHS, bfe, by = "lonlat")
  ...
  # here that's normal that they don't have the same number of rows, because they are not based on the same grid cell selection in the first place. 
  # However, this is not a problem as we merge on geographical information. 
  ...
  saveRDS(parcels, file.path(paste0("temp_data/panel_parcels_ip_final_",
                                    parcel_size/1000,"km_",
                                    catchment_radius/1000,"CR_CA2.rds")))  
