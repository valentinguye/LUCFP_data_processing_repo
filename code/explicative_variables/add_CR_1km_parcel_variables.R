...
make_n_reachable_uml <- function(parcel_size, catchment_radius){
  ...
  parcels <- readRDS(file.path(paste0("temp_data/processed_parcels/lucpfip_panel_dynamics_",
...
for(catchment_radius in catchment_radiuseS){ 
  ...
  variables <- c("wa_ffb_price_imp1", "wa_cpo_price_imp1") # those necessary for controls in IV lagged strategy
...
make_spatial_ov_lags <- function(catchment_radius){
  ...
  LHS <- inner_join(lucpfip_dyn, lucpfsmp, by = c("lonlat", "year")) 
